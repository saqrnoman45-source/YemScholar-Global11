...
async function serializeArticle(a: typeof articlesTable.$inferSelect) {
  ...
  return {
    id: a.id, title: a.title, summary: a.summary, content: a.content, topic: a.topic,
    authorId: a.authorId, authorName: author?.name ?? "Unknown",
    thumbnailUrl: a.thumbnailUrl ?? null, readTime: a.readTime, publishedAt: a.publishedAt.toISOString(),
  };
}
...
// Teacher/Admin: publish article
router.post("/articles", requireAuth, requireRole("teacher", "admin"), async (req, res): Promise<void> => {
  const parsed = CreateArticleBody.safeParse(req.body);
  if (!parsed.success) { res.status(400).json({ error: parsed.error.message }); return; }
  const [article] = await db.insert(articlesTable).values({ ...parsed.data, authorId: req.user!.id }).returning();
...
// Teacher/Admin: update article
router.patch("/articles/:id", requireAuth, requireRole("teacher", "admin"), async (req, res): Promise<void> => {
  const raw = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
...
router.delete("/articles/:id", requireAuth, requireRole("admin"), async (req, res): Promise<void> => {
  const raw = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
