...
router.patch("/admin/users/:id", requireAuth, requireRole("admin"), async (req, res): Promise<void> => {
  ...
  res.json(UpdateAdminUserResponse.parse({ id: user.id, email: user.email, name: user.name, role: user.role, bio: user.bio ?? null, avatarUrl: user.avatarUrl ?? null, createdAt: user.createdAt.toISOString() }));
});

router.get("/admin/applications", requireAuth, requireRole("admin"), async (_req, res): Promise<void> => {
  ...
  const result = rows.map(r => ({
    ...
    scholarship: r.scholarships ? {
      id: r.scholarships.id, title: r.scholarships.title, description: r.scholarships.description,
      provider: r.scholarships.provider, amount: r.scholarships.amount ?? null, category: r.scholarships.category,
      deadline: r.scholarships.deadline?.toISOString() ?? null, eligibility: r.scholarships.eligibility,
      applicationCount: r.scholarships.applicationCount ?? 0, createdAt: r.scholarships.createdAt.toISOString(),
    } : null,
  }));
  res.json(ListAdminApplicationsResponse.parse(result));
});

router.patch("/admin/applications/:id", requireAuth, requireRole("admin"), async (req, res): Promise<void> => {
  const raw = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
  ...
  res.json(UpdateApplicationStatusResponse.parse({
    id: application.id, userId: application.userId, scholarshipId: application.scholarshipId,
    status: application.status, statement: application.statement ?? null, appliedAt: application.appliedAt.toISOString(),
    scholarship: scholarship ? {
      id: scholarship.id, title: scholarship.title, description: scholarship.description,
      provider: scholarship.provider, amount: scholarship.amount ?? null, category: scholarship.category,
      deadline: scholarship.deadline?.toISOString() ?? null, eligibility: scholarship.eligibility,
      applicationCount: scholarship.applicationCount ?? 0, createdAt: scholarship.createdAt.toISOString(),
    } : null,
  }));
});
