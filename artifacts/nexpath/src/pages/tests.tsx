...
import { FlaskConical, Clock, CheckCircle, XCircle, RotateCcw, ChevronRight, Trophy, Target } from "lucide-react";
    ...
    questions: [
      {
        ...
        options: ["Machine Language", "Machine Learning", "Managed Logic", "Meta Learning"],
        correct: 1,
        explanation: "ML stands for Machine Learning — a subset of AI where systems learn from data.",
      },
      {
        id: 2,
        question: "Which of the following is a supervised learning algorithm?",
        options: ["K-Means Clustering", "DBSCAN", "Linear Regression", "Principal Component Analysis"],
        correct: 2,
        explanation: "Linear Regression is supervised because it trains on labeled input-output pairs.",
      },
      {
        id: 3,
        ...
        correct: 1,
        explanation: "Overfitting occurs when a model fits training data too closely, hurting generalisation.",
      },
      {
        id: 4,
        ...
        correct: 2,
        explanation: "The Sigmoid function maps any real value to the range (0, 1), making it useful for probabilities.",
      },
    ],
  },
  {
    ...
    questions: [
      {
        ...
        correct: 3,
        explanation: "GET is the HTTP method for retrieving data without modifying server state.",
      },
      {
        id: 2,
    ...
    questions: [
      {
        ...
        correct: 2,
        explanation: "A balanced BST has height O(log n), so search takes O(log n) time.",
      },
      {
        id: 2,
        ...
        correct: 1,
        explanation: "A Queue follows First-In-First-Out — the first element added is the first removed.",
      },
      {
        id: 3,
        ...
        correct: 2,
        explanation: "QuickSort degrades to O(n²) in the worst case, e.g. when the pivot is always the smallest or largest element.",
      },
      {
        id: 4,
    ...
    <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5 hover:border-zinc-600 hover:shadow-lg hover:shadow-black/30 transition-all flex flex-col gap-4">
      ...
      <div className="flex items-center gap-4 text-xs text-zinc-500">
        <span className="flex items-center gap-1.5">
          <Target className="w-3.5 h-3.5" />{quiz.questions.length} questions
        </span>
        <span className="flex items-center gap-1.5">
...
function QuizRunner({ quiz, onFinish }: { quiz: Quiz; onFinish: (score: number, total: number) => void }) {
          ...
          {q.options.map((option, idx) => {
            ...
            if (revealed) {
              if (idx === q.correct) {
                ...
                style = "border-zinc-700/30 bg-zinc-800/20 text-zinc-500";
              }
...
function Results({ score, total, quizTitle, onRestart, onBack }: {
  score: number; total: number; quizTitle: string; onRestart: () => void; onBack: () => void;
}) {
    ...
    <div className="max-w-md mx-auto text-center">
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-8">
        ...
        <div className="h-2 bg-zinc-800 rounded-full overflow-hidden mb-6">
          <div
            className="h-full bg-gradient-to-r from-violet-500 to-sky-500 rounded-full transition-all duration-1000"
            style={{ width: `${pct}%` }}
          />
        </div>
...
export default function Tests() {
  ...
  const [result, setResult] = useState<{ score: number; total: number; title: string } | null>(null);

  const handleFinish = (score: number, total: number) => {
    setResult({ score, total, title: activeQuiz!.title });
    setActiveQuiz(null);
  };
      ...
      <div className="p-6">
        {result ? (
          <Results
            ...
            onRestart={() => {
              const q = quizzes.find((q) => q.title === result.title);
